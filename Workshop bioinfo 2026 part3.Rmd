...
cran_packages <- c("knitr","factoextra", "gridExtra", "ggplot2", "psych", "diptest", "MASS", "pheatmap", "dplyr")
...
Annotating methylKit CpGs for Kryptolebias marmoratus using a custom GTF
1. Build the TxDb from your GTF
Since there is no pre-built Bioconductor package for this species, this is the critical first step.
...
txdb <- makeTxDbFromGFF(
  file       = "Sequences/genomic.gtf",
  format     = "gtf",
  organism   = "Kryptolebias marmoratus",
  dataSource = "custom"
)

# Save to avoid rebuilding every session
saveDb(txdb, "Sequences/Kmarmoratus_txdb.sqlite")

# Reload in future sessions
# txdb <- loadDb("Kmarmoratus_txdb.sqlite")
...
# Convert methylKit object to GRanges
cpg_gr <- as(all.diff, "GRanges")
cpg_gr

# Annotate
peakAnno <- annotatePeak(
  cpg_gr,
  tssRegion   = c(-2000, 200),   # define promoter window
  TxDb        = txdb,
...
plotDistToTSS(peakAnno, title = "CpG distribution relative to TSS")

# Export table
anno_df <- as.data.frame(peakAnno)
anno_df
write.csv(anno_df, "Raw_Data_Florida_Belize/results/CpG_Kmarmoratus_annotated.csv", row.names = FALSE)

# Keep only promoter CpGs
promoter_cpg <- anno_df[grep("Promoter", anno_df$annotation), ]
promoter_cpg

# Keep only intergenic
intergenic_cpg <- anno_df[grep("Distal Intergenic", anno_df$annotation), ]

# Keep CpGs within 2kb of TSS
near_tss <- anno_df[abs(anno_df$distanceToTSS) <= 2000, ]


# Unique genes hit by your differentially methylated CpGs
gene_list <- unique(anno_df$geneId)
length(gene_list)
gene_list

write.csv(data.frame(gene = gene_list), 
          "Raw_Data_Florida_Belize/results/genes_associated_DMCpGs.csv", 
          row.names = FALSE)
```

4. GO enrichment from your gene list (bonus)

```{r}
install.packages("gprofiler2")
library(gprofiler2)

# Direct GO enrichment with K. marmoratus gene IDs
gost_results <- gost(
  query         = gene_list,
  organism      = "kmarmoratus",  # check exact ID on gprofiler
  ordered_query = FALSE,
...
# Export table
result_df <- gost_results$result

# Convert list-columns to semicolon-separated strings
result_df$parents <- sapply(result_df$parents, 
                             function(x) paste(unlist(x), collapse = ";"))

result_df$intersection <- sapply(result_df$intersection,
                                  function(x) paste(unlist(x), collapse = ";"))

write.csv(result_df, "Raw_Data_Florida_Belize/results/GO_enrichment_Kmarmoratus.csv", row.names = FALSE)
...
ggplot(anno_df, aes(x = distanceToTSS)) +
  geom_histogram(bins = 100, fill = "#4fc3d8", color = "white", linewidth = 0.2) +
  scale_x_continuous(limits = c(-50000, 50000)) +
  labs(title = "CpG distance to nearest TSS",
       x = "Distance to TSS (bp)", y = "Count") +
  theme_minimal()


# Simplify annotation categories
anno_df$feature <- gsub("Promoter.*", "Promoter", anno_df$annotation)
anno_df$feature <- gsub("Exon.*",     "Exon",     anno_df$feature)
anno_df$feature <- gsub("Intron.*",   "Intron",   anno_df$feature)

ggplot(anno_df, aes(x = reorder(feature, meth.diff), 
                     y = meth.diff, fill = feature)) +
  geom_violin(alpha = 0.7) +
  geom_boxplot(width = 0.1, outlier.size = 0.5) +
  labs(title = "Methylation difference by genomic feature",
       x = "", y = "Methylation difference (%)") +
  theme_minimal() +
  theme(legend.position = "none")


ggplot(anno_df, aes(x = seqnames)) +
  geom_bar(fill = "#4fc3d8") +
  labs(title = "DMCpGs per chromosome",
...
We can instead use the annotation made in SeqMonk. Here we upload the .CSV file generated by SeqMonk.

SeqMonk is a quality control and exploration tool, not an analytical endpoint. If you're satisfied with your statistical results and the ChIPseeker annotation makes biological sense, you can skip SeqMonk entirely and go straight to writing up. It's most valuable early in the pipeline to sanity-check your data, or at the end to generate genome browser-style figures for a paper.
