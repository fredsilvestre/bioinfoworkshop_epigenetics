---
title: "DNA methylation workflow - workshop 2025 part2"
...
bioc_packages <- c("GenomicRanges", "GenomeInfoDb", "IRanges", "Biostrings", "BSgenome", "AnnotationHub", "GenomicFeatures", "SGSeq", "qvalue", "pwalign", "methylKit", "ShortRead","genomation", "remotes", "factoextra", "gridExtra", "ggplot2", "txdbmaker", "psych", "diptest")  
...
The location is: [1] "/Library/Frameworks/R.framework/Versions/4.4/Resources/library/methylKit/extdata/test1.myCpG.txt"
...
We can search for SMVs using the interquartile range method.
...
pcs_df <- data.frame(as$pcs, batch_id = sampleAnnotation$batch_id)
ggplot(pcs_df, aes(x = PC1, y = PC2, color = batch_id)) +
  geom_point(size = 3) +
  labs(title = "PCA of Methylation Data", x = "PC1", y = "PC2") +
  theme_minimal()

pcs_df <- data.frame(as$pcs, batch_id = sampleAnnotation$age)
ggplot(pcs_df, aes(x = PC1, y = PC2, color = batch_id)) +
  geom_point(size = 3) +
  labs(title = "PCA of Methylation Data", x = "PC1", y = "PC2") +
  theme_minimal()
...
tiles = tileMethylCounts(myobj_lowCov,win.size=1000,step.size=1000,cov.bases = 0)
tiles
?tileMethylCounts
...
#How many CpGs are in exons of test 1 ?
